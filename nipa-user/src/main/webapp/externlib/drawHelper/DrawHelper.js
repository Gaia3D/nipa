        ...
        if(callbacks) {
            ...
            if(callbacks.dragHandlers) {
                ...
                setListener(billboard, 'leftDown', function(position) {
                    ...
                    callbacks.dragHandlers.onDragStart && callbacks.dragHandlers.onDragStart(getIndex(), _self._scene.pickPosition(position));
                });
            }
        ...
        mouseHandler.setInputAction(function(movement) {
            if(movement.position != null) {
                ...
                //if (cartesian) {
					...
					//m###
					if (isDrawFeature.isDraw && isDrawFeature.type == 'polyline') {
						var coordinates = [];
						var pos = ellipsoid.cartesianToCartographic(cartesian);
						coordinates.push(Cesium.Math.toDegrees(pos.longitude));
						coordinates.push(Cesium.Math.toDegrees(pos.latitude));
        ...
        function updateExtent(value) {
            if(extent == null) {
                ...
                extent = new Cesium.Primitive({
                    geometryInstances :  new Cesium.GroundPrimitive()
            });
            extent.asynchronous = false;
            primitives.add(extent);
            }
    ...
    _.prototype.enhancePrimitives = function() {
        ...
        Cesium.Billboard.prototype.setEditable = function() {
            ...
            setListener(billboard, 'leftDown', function(position) {
                ...
                handler.setInputAction(function(movement) {
                    //var cartesian = drawHelper._scene.camera.pickEllipsoid(movement.position, ellipsoid);
                    var cartesian = drawHelper._scene.pickPosition(movement.position);
                    if (Cesium.defined(cartesian)) {
                    //if (cartesian) {
                        onDrag(cartesian);
                    } else {
                ...
                handler.setInputAction(function(movement) {
                    ...
                    onDragEnd(drawHelper._scene.pickPosition(movement.position));
                }, Cesium.ScreenSpaceEventType.LEFT_UP);
